...
async def _reset_database_state(postgres_session_factory):
    # Qualify each table with its schema so tables living outside the
    # default ``public`` schema (e.g. EvoSport's ``evosport`` schema) are
    # truncated with the correct relation name.  Without the schema
    # qualifier PostgreSQL looks only in ``public`` and raises
    # UndefinedTableError once any test module registers the EvoSport
    # model into the shared Base.metadata (making the table appear in
    # ``sorted_tables`` even though it lives in ``evosport``).
    table_names = [
        f"{_quote_ident(table.schema)}.{_quote_ident(table.name)}" if table.schema else _quote_ident(table.name)
